...
power_digits=fu_Digits(max(max(results.MainFreqPower)));
...
figurehandle(length(info.phaselength)+1,1)=fig_Timeseries(wavelet_Xaxis, results.wavelet.HzBIN, results.wavelet.MW,... & wavelet
                        [1*info.binsize:info.binsize:totalTime_min], results.MainFreq, results.GammaPower, results.FullWidth_at_HalfMax,...
