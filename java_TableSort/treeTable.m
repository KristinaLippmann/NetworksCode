function [jtable, paramsStruct] = treeTable(varargin)
...
% $Revision: 1.4 $  $Date: 2013/08/06 14:31:16 $
  ...
  %try
      ...
      try
          ...
          set(hcontainer,'units','normalized','pos',[0,0.1,1,0.8]);  % this will resize the table whenever its container is resized
          pause(0.05);
      catch
          err = lasterror;
          hcontainer = [];
...
function tableChangedCallback(hModel,hEvent,jtable)
    ...
    oldDATA=get(Figure_handle, 'UserData');
    
    if isstruct(oldDATA)
        oldoutput=get(Figure_handle, 'UserData');
        oldDATA=oldoutput.output;
    end
    
    oldDATA{modifiedRow+1,modifiedCol+1}=newData;
    
    set(Figure_handle, 'UserData' ,oldDATA)
