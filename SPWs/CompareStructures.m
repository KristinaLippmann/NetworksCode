function varargout = CompareStructures(S1, S2,P)
% COMPARESTRUCTURES Lists differences in fields of two structures
%  CompareStructures(S1,S2,P) will display the fields and values of the two
...
%  [Feq, Fneq, FonlyS1, FonlyS2] = CompareStructures(S1,S2,P) will
...
%
% Added by Jens: input 'P', which flags: 0: only errors are printed,
% 1  everything is printed to the command window
%
...
nomatch = [];
for ii=1:length(fic)
    if isequal(S1.(fic{ii}),S2.(fic{ii}))
        match(end+1) = ii; %#ok<AGROW>
    else
        nomatch(end+1) = ii; %#ok<AGROW>
    end
end
if P
    if ~isempty(fic)
...
if numel(vec) == 1
    ...
    if length(vec) > 1
        for ii=2:length(vec)
            str = [str 'x' num2str(vec(ii))]; %#ok<AGROW>
        end
    end
end
str = ['[' str ']'];
