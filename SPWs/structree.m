...
while ~isequal(level,0)                                                   
    ...
    if (flag==1)                                                    
        if(row(level)==1)  %do this only if you are accesing this level for the first time
            ...
            Directory_size(level)=Temp_size(1);             %#ok<AGROW> %store number of fields
            Directory_names(1:Directory_size(level),level)=Temp_names;  %#ok<AGROW> %store field names in corresponding level
        end
        ...
        if (level>1)
            structure(level,1).data=getfield(structure(level-1,1).data,Directory_names{row(level-1),level-1}); %#ok<GFLD> %Update structure
        end 
        
    end
end
status = fclose(fid);  %#ok<NASGU> %Close file
