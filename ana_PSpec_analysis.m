...
% param(9): relative gamma power (30-100 Hz)
% param(10): relative lowfrequency gamma power (30-50 Hz)
% param(11): relative midfrequency gamma power (50-90 Hz)
if ~artefact
%% no smoothing
% % smooth the spectrum
...
g = x1>=30 & x1<=100;
g = g(1:end-1);
powerparams(9)=(100.*trapz(y1(g)))./p;

%low frequency gamma power (30-50 Hz)
l = x1>=30 & x1<=50; 
powerparams(10)=(100.*trapz(y1(l)))./p;

%mid frequency gamma power (50-90 Hz)
m = x1>=50 & x1<=90;
powerparams(11)=(100.*trapz(y1(m)))./p;

clear p g l m
else
    powerparams(1,1:11)=NaN;
end
